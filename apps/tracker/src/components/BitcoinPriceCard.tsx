          ...
          {price ? (
            <>
              ...
              {price.changeRate && price.changePrice && (
                <div
                  className={`flex flex-col sm:flex-row sm:items-center sm:space-x-2 space-y-1 sm:space-y-0 ${
                    price.change === "RISE" ? "text-red-500" : "text-blue-500"
                  }`}
                >
              ...
              {price.volume && (
                <div className="text-xs text-muted-foreground">
                  24시간 거래량:{" "}
                  <span className="font-medium">
                    {price.volume.toFixed(2)} BTC
                  </span>
                </div>
              )}

              <p className="text-xs text-muted-foreground">
                마지막 업데이트: {formatLastUpdate(price.timestamp)} (업비트)
              </p>
            </>
          ) : (
            <div className="space-y-2">
