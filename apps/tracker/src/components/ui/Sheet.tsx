...
const SheetOverlay = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof SheetPrimitive.Overlay>>(
  ({ className, ...props }, ref) => <SheetPrimitive.Overlay className={cn("fixed inset-0 z-50 bg-black/50 transition-opacity duration-300", className)} {...props} ref={ref} />
);
SheetOverlay.displayName = SheetPrimitive.Overlay.displayName;

const sheetVariants = cva("fixed z-50 bg-white dark:bg-gray-900 p-6 shadow-xl transition-transform duration-300 ease-in-out border border-gray-200 dark:border-gray-700", {
  variants: {
    side: {
