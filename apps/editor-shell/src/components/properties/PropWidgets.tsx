...
export function PropGrid({ children }: { children: React.ReactNode }) {
  return <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, padding: "0 14px" }}>{children}</div>
}
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <select
        ...
        style={{
          ...
          border: `1px solid ${T.inputBorder}`,
          borderRadius: 6,
          color: mixed ? T.textMuted : T.text,
          fontSize: 12,
          minWidth: 0,
          outline: "none",
          cursor: "pointer",
...
export function PropToggleRow({ label, value, onChange }: { label: string; value: boolean; onChange: (v: boolean) => void }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{ fontSize: 12, color: T.textSub, width: 56, flexShrink: 0 }}>{label}</span>
      <input type="checkbox" checked={value} onChange={(e) => onChange(e.target.checked)} style={{ accentColor: T.accent, cursor: "pointer" }} />
    </div>
  )
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6 }}>
        {color && !isMixed && <ColorPickerPopover value={String(value)} onChange={onChange} onLiveChange={onLiveChange} />}
        <input
          ...
          style={{
            ...
            border: `1px solid ${T.inputBorder}`,
            borderRadius: 6,
            color: isMixed ? T.textMuted : T.text,
            fontSize: 12,
            minWidth: 0,
            outline: "none",
          }}
