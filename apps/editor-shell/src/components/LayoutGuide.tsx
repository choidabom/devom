...
const REGIONS = [
  ...
  },
  {
    ...
    color: "249, 115, 22",
    lines: [
      "selectionStore.selectedElement를 MobX observer로 구독 → 선택 변경 시 자동 갱신",
      "input 값 변경 → historyStore.pushSnapshot() → documentStore.updateStyle()",
      "숫자 문자열은 Number()로 파싱 → CSS 값으로 적용",
      "sc: 접두사 요소는 CSS 대신 React props(variant, label 등) 편집 UI 표시",
      "모든 변경 → toSerializable() → SYNC_DOCUMENT로 Canvas 전체 동기화 (MobX proxy 이슈 우회)",
    ],
  ...
  const cards = [
    {
      ...
      sections: [
        { h: "Stores (MobX)", items: [
          ...
          "SelectionStore — 현재 선택된 요소 ID + bounds 추적",
    ...
    },
    {
      ...
      sections: [
        ...
        { h: "메시지 흐름", items: [
          "Shell→Canvas: SYNC_DOCUMENT, ADD_ELEMENT, SELECT_ELEMENT",
          "Canvas→Shell: CANVAS_READY, ELEMENT_CLICKED, MOVED, RESIZED, KEY_EVENT",
        ]},
      ],
    },
    {
      ...
      sections: [
        ...
        { h: "인터랙션", items: [
          ...
          "키보드: iframe 내 keydown → KEY_EVENT 메시지로 Shell에 전달",
        ]},
      ],
    },
  ]
