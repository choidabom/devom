...
export const CardContent = memo(({ item }: CardContentProps) => {
  const renderContent = (): ReactNode => {
    if (item.contentType === "image" && item.img_url) {
      return <img src={item.img_url} alt={item.id} width={item.width} height={item.height} draggable={false} className="card-image" />
    }
