...
export const ApplicationProvider = ({ children }: { children: ReactNode }): ReactNode => {
  ...
  const addApplication = useCallback((name: string, component: ReactNode, config: Omit<Application["config"], "zIndex">) => {
  ...
  return <ApplicationContext.Provider value={{ applications, addApplication, setZIndexToFront }}>{children}</ApplicationContext.Provider>;
};
