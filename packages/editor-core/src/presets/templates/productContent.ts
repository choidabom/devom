...
export function buildProductContent(store: DocumentStore): void {
  ...
  if (keyVisual) {
    ...
    add("text", keyVisual, {
      name: "KV Title",
      style: { ...rel, fontSize: 36, fontWeight: 800, color: t, textAlign: "center", lineHeight: 1.3 },
      props: { content: "프리미엄 무선 헤드폰\nANC-Pro 3세대" },
    })
    add("text", keyVisual, {
      name: "KV Sub",
      style: { ...rel, fontSize: 16, color: s, textAlign: "center", lineHeight: 1.6 },
      props: { content: "노이즈캔슬링 · 40시간 배터리 · Hi-Res 오디오" },
    })
  ...
  if (uspSection) {
    ...
    const usps = [
      {
        title: "하이브리드 노이즈캔슬링",
        desc: "외부 마이크 4개가 주변 소음을 실시간 분석하여 99%까지 차단합니다. 비행기, 카페, 사무실 어디서든 나만의 조용한 공간을 만들어 보세요.",
        color: "#f0f4ff",
      },
      {
        title: "40시간 초장시간 재생",
        desc: "ANC 켠 상태에서도 40시간 연속 재생. 10분 급속 충전으로 3시간 사용 가능해 배터리 걱정 없이 하루를 보낼 수 있습니다.",
        color: "#f0fdf4",
      },
  ...
  if (certSection) {
    ...
    const certGrid = add("div", certSection, {
      ...
      style: { ...rel, width: "auto", height: "auto", ...box },
      layoutMode: "flex",
      layoutProps: { direction: "row", gap: 24, ...noPad, alignItems: "center", justifyContent: "center" },
      sizing: { w: "fill", h: "hug" },
    })
    if (certGrid) {
      const certs = [
      ...
      for (const c of certs) {
        const certCard = add("div", certGrid, {
          ...
          style: { ...rel, width: "auto", height: "auto", ...box },
          layoutMode: "flex",
          layoutProps: { direction: "column", gap: 8, ...noPad, alignItems: "center" },
          sizing: { w: "hug", h: "hug" },
        })
        if (certCard) {
          add("div", certCard, {
          ...
          add("text", certCard, {
            name: `${c.label.split("\n")[0]} L`,
            style: { ...rel, fontSize: 11, color: s, textAlign: "center", lineHeight: 1.4 },
            props: { content: c.label },
          })
        }
      }
    }
