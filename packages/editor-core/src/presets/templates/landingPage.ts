...
export function buildLandingPage(store: DocumentStore): void {
  ...
  if (topBar) {
    add("text", topBar, {
      ...
      props: { content: "🚀 오늘 주문 시 내일 도착 | 3만원 이상 무료배송" },
    })
  }
  ...
  if (hero) {
    ...
    if (info) {
      ...
      add("text", info, {
        ...
        props: { content: "프리미엄 노이즈캔슬링 무선 헤드폰 ANC-Pro 3세대" },
      })
      ...
      if (priceSection) {
        ...
        const benefitRow = add("div", priceSection, {
          ...
          style: { ...rel, width: "auto", height: "auto", ...box },
          layoutMode: "flex",
          layoutProps: { direction: "row", gap: 4, ...noPad, alignItems: "center" },
          sizing: { w: "hug", h: "hug" },
        })
        if (benefitRow) {
      ...
      if (deliveryInfo) {
        ...
        for (const [label, value] of rows) {
          const dr = add("div", deliveryInfo, {
            ...
            style: { ...rel, width: "auto", height: "auto", ...box },
            layoutMode: "flex",
            layoutProps: { direction: "row", gap: 12, ...noPad, alignItems: "center" },
            sizing: { w: "fill", h: "hug" },
          })
          if (dr) {
  ...
  if (detailSection) {
    ...
    add("text", detailSection, {
      name: "Detail Title",
      style: { ...rel, fontSize: 28, fontWeight: 700, color: t, textAlign: "center", lineHeight: 1.4 },
      props: { content: "당신의 일상에 조용한 혁신을\n가져다 줄 헤드폰" },
    })
    add("text", detailSection, {
      name: "Detail Subtitle",
      style: { ...rel, fontSize: 15, color: s, textAlign: "center", lineHeight: 1.6 },
      props: { content: "프리미엄 ANC 기술과 40시간 배터리로\n어디서든 최고의 사운드를 경험하세요." },
    })
  ...
  if (reviewSection) {
    ...
    const reviewHeader = add("div", reviewSection, {
      ...
      style: { ...rel, width: "auto", height: "auto", ...box },
      layoutMode: "flex",
      layoutProps: { direction: "row", gap: 16, ...noPad, alignItems: "end", justifyContent: "space-between" },
      sizing: { w: "fill", h: "hug" },
    })
    if (reviewHeader) {
    ...
    if (ratingBar) {
      ...
      const barCol = add("div", ratingBar, {
        ...
        style: { ...rel, width: "auto", height: "auto", ...box },
        layoutMode: "flex",
        layoutProps: { direction: "column", gap: 4, ...noPad, alignItems: "stretch" },
        sizing: { w: "fill", h: "hug" },
      })
      if (barCol) {
        for (const [star, pct] of [
          ["5점", 92],
          ["4점", 5],
          ["3점", 2],
          ["2점", 1],
    ...
    const reviews = [
      {
        ...
        date: "2026.03.10",
        rating: "★★★★★",
        text: "노이즈캔슬링 성능이 정말 대단합니다. 지하철에서 음악 듣는데 주변 소리가 거의 안 들려요. 배터리도 이틀 연속 사용해도 넉넉합니다.",
        tag: "음질 좋아요",
      },
      {
        name: "박**",
        date: "2026.03.08",
        rating: "★★★★★",
        text: "직장에서 하루종일 착용해도 귀 안 아파요. 메모리폼 쿠션이 진짜 부드럽고 250g이라 가벼워서 장시간 착용 가능합니다.",
        tag: "착용감 최고",
      },
      {
        name: "이**",
        date: "2026.03.05",
        rating: "★★★★☆",
        text: "음질이랑 노이즈캔슬링은 만족인데 케이스가 좀 컸으면 좋겠어요. 그래도 가성비는 이 가격대 최고입니다.",
        tag: "가성비 굿",
      },
    ]
  ...
  if (returnSection) {
    ...
    add("sc:alert", returnSection, {
      ...
      props: {
        title: "안내",
        description: "제품 하자로 인한 교환/반품 시 택배비는 판매자가 부담합니다. 단순 변심에 의한 교환/반품 시 택배비는 구매자 부담입니다.",
        variant: "default",
      },
      sizing: { w: "fill", h: "hug" },
    })
  ...
  if (footer) {
    ...
    add("text", footer, {
      name: "Footer Info",
      style: { ...rel, fontSize: 11, color: "#888888", lineHeight: 1.8 },
      props: { content: "(주)데봄스토어 | 대표: 홍길동 | 사업자등록번호: 123-45-67890\n서울특별시 강남구 테헤란로 123, 4층\n고객센터: 1588-0000 (평일 09:00~18:00)" },
    })
    add("sc:separator", footer, { name: "Footer Sep", style: { ...rel, width: undefined, borderColor: "#333333" }, sizing: { w: "fill", h: "fixed" } })
    add("text", footer, {
      name: "Copyright",
      style: { ...rel, fontSize: 11, color: "#666666" },
      props: { content: "© 2026 DEVOM STORE. All rights reserved. | 개인정보처리방침 | 이용약관" },
    })
  }
}
