...
export function buildFoodProduct(store: DocumentStore): void {
  ...
  if (topBar) {
    add('text', topBar, { name: 'Top Notice', style: { ...rel, fontSize: 12, fontWeight: 500, color: '#ffffff' }, props: { content: '🧊 새벽배송 마감 오늘 밤 11시 | 첫 주문 시 5,000원 할인' } })
  }
  ...
  if (hero) {
    ...
    if (imgCol) {
      ...
      const thumbRow = add('div', imgCol, {
        name: 'Thumbnails',
        style: { ...rel, width: 'auto', height: 'auto', ...box },
        layoutMode: 'flex', layoutProps: { direction: 'row', gap: 6, ...noPad, alignItems: 'center' },
        sizing: { w: 'fill', h: 'hug' },
      })
      if (thumbRow) {
        for (let i = 1; i <= 5; i++) {
          add('div', thumbRow, {
            name: `Thumb ${i}`,
            style: { ...rel, width: 64, height: 64, backgroundColor: i === 1 ? '#fef3e0' : '#f5f5f5', borderRadius: 4, border: i === 1 ? `2px solid ${purple}` : `1px solid ${bd}` },
    ...
    if (info) {
      ...
      add('text', info, { name: 'Product Title', style: { ...rel, fontSize: 22, fontWeight: 700, color: t, lineHeight: 1.4 }, props: { content: '[산지직송] 경북 영주 꿀사과\n프리미엄 선물세트 3kg (9~11과)' } })
      ...
      if (delInfo) {
        ...
        for (const [label, value] of rows) {
          const dr = add('div', delInfo, {
            name: `Del ${label}`,
            style: { ...rel, width: 'auto', height: 'auto', ...box },
            layoutMode: 'flex', layoutProps: { direction: 'row', gap: 12, ...noPad, alignItems: 'start' },
            sizing: { w: 'fill', h: 'hug' },
          })
          if (dr) {
      ...
      const totalRow = add('div', info, {
        ...
        layoutMode: 'flex', layoutProps: { direction: 'row', gap: 8, paddingTop: 12, paddingRight: 16, paddingBottom: 12, paddingLeft: 16, alignItems: 'center', justifyContent: 'space-between' },
        sizing: { w: 'fill', h: 'hug' },
      })
      if (totalRow) {
  ...
  if (noticeBanner) {
    ...
    add('text', noticeBanner, { name: 'Fresh Text', style: { ...rel, fontSize: 13, color: green, fontWeight: 600, lineHeight: 1.5 }, props: { content: '이 상품은 신선식품으로 단순 변심에 의한 교환/반품이 어렵습니다. 상품 설명을 꼼꼼히 확인해주세요.' } })
  }
  ...
  if (detailSection) {
    // Hero text
    add('text', detailSection, { name: 'Detail Hero', style: { ...rel, fontSize: 26, fontWeight: 700, color: t, textAlign: 'center', lineHeight: 1.5 }, props: { content: '해발 300m 고랭지에서 자란\n당도 14Brix 프리미엄 꿀사과' } })
    add('text', detailSection, { name: 'Detail Sub', style: { ...rel, fontSize: 14, color: s, textAlign: 'center', lineHeight: 1.7 }, props: { content: '경북 영주의 깨끗한 공기와 일교차가 만들어낸\n아삭하고 달콤한 최고급 사과를 만나보세요.' } })
  ...
  if (reviewSection) {
    ...
    const rHeader = add('div', reviewSection, {
      name: 'Review Header',
      style: { ...rel, width: 'auto', height: 'auto', ...box },
      layoutMode: 'flex', layoutProps: { direction: 'row', gap: 12, ...noPad, alignItems: 'end', justifyContent: 'space-between' },
      sizing: { w: 'fill', h: 'hug' },
    })
    if (rHeader) {
    ...
    // Reviews
    const reviews = [
      { name: '최**', date: '2026.03.11', rating: '★★★★★', text: '부모님 생신 선물로 보내드렸는데 너무 좋아하셨어요! 사과 하나하나 크기도 균일하고 당도가 정말 높아서 생과일주스로도 딱이에요. 포장도 꼼꼼하게 되어있었습니다.', tag: '선물용 좋아요', hasPhoto: true },
      { name: '정**', date: '2026.03.09', rating: '★★★★★', text: '3번째 재구매입니다. 마트에서 사는 것보다 훨씬 맛있고 신선해요. 아이들이 사과를 안 좋아했는데 이 사과는 잘 먹어요. 당도 보장이라 그런지 진짜 달아요.', tag: '재구매 의사' },
      { name: '한**', date: '2026.03.07', rating: '★★★★☆', text: '전체적으로 만족합니다. 사과 크기가 조금 들쭉날쭉한 게 아쉽지만 맛은 확실해요. 아삭하고 당도 높고요. 배송도 빨라서 좋았습니다.', tag: '당도 높아요' },
    ]
  ...
  if (returnSection) {
    ...
    add('sc:alert', returnSection, {
      name: 'Fresh Notice',
      style: { ...rel, width: undefined },
      props: { title: '신선식품 안내', description: '본 상품은 신선식품으로 단순 변심에 의한 교환/반품이 불가합니다. 수령 즉시 상태를 확인해주시고, 문제 발생 시 사진과 함께 고객센터로 연락해주세요.', variant: 'default' },
      sizing: { w: 'fill', h: 'hug' },
    })
  }
  ...
  if (footer) {
    ...
    add('text', footer, { name: 'Footer Info', style: { ...rel, fontSize: 11, color: '#aaaaaa', lineHeight: 1.8 }, props: { content: '(주)프레시마켓 | 대표: 김신선 | 사업자등록번호: 987-65-43210\n서울특별시 송파구 올림픽로 456, 3층\n통신판매업신고: 제2026-서울송파-1234호\n고객센터: 1588-1234 (평일 09:00~18:00)' } })
    add('sc:separator', footer, { name: 'Footer Sep', style: { ...rel, width: undefined, borderColor: '#555555' }, sizing: { w: 'fill', h: 'fixed' } })
    add('text', footer, { name: 'Copyright', style: { ...rel, fontSize: 11, color: '#888888' }, props: { content: '© 2026 FRESH MARKET. All rights reserved. | 개인정보처리방침 | 이용약관 | 사업자정보확인' } })
  }
}
